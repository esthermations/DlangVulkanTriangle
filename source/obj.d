...
ObjData parseObj(string objFilePath)
{
   ...
   Vector!(float, N) stringsToVec(int N)(string[] splitString)
   {
      ...
      foreach (i; 0 .. N)
      {
         ret.data[i] = splitString[i].to!float;
      }
      return ret;
   }
   ...
   foreach (splitLine; objFile.byLineCopy
      .map!(strip)
      .map!(split))
   {
      ...
      {
      case "v":
         uniquePositions ~= stringsToVec!3(rest);
         break;
      case "vn":
         uniqueNormals ~= stringsToVec!3(rest);
         break;
      case "vt":
         uniqueTexcoords ~= stringsToVec!2(rest);
         break;
