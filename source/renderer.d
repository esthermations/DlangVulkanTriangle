...
struct Vertex
{
   ...
   v3 position; /// Model-space position of this vertex
   v3 normal; /// Vertex normal vector
...
public:
   ...
   out (; m_state == RendererState.BetweenFrames)
   {
      ...
      VkSubmitInfo submitInfo = {
         ...
         pWaitSemaphores: &this.swapchain.imageAvailableSemaphore.m_semaphore,// Wait until image is available. This resets the semaphore when
         // done waiting.
      ...
      // command buffer.
      check!vkQueueSubmit(this.graphicsQueue, 1, &submitInfo, this.swapchain
            .commandBufferReadyFence);
      ...
      VkPresentInfoKHR presentInfo = {
         waitSemaphoreCount: 1,
         pWaitSemaphores: &this.swapchain.renderFinishedSemaphore.m_semaphore,// Wait for rendering to be done before presenting the image. This
   ...
   VkSwapchainKHR CreateVkSwapchain(GLFWwindow* window, VkPhysicalDevice physicalDevice, VkSurfaceKHR surface)
   {
   ...
   struct Swapchain
   {
      ...
      {
         SendNewData(commandBuffers[imageIndex], uniformBuffers[imageIndex], [
               data
            ]);
      }
   ...
   Swapchain createSwapchain()
   {
      ...
      ret.renderPass = CreateRenderPass(
         this.logicalDevice,
         SelectSurfaceFormat(
            SwapchainSupportDetails.Get(this.physicalDevice, this.surface).formats).format,
         kDepthFormat
      );

      ret.pipeline = this.createGraphicsPipeline(ret.renderPass);
      ret.depthResources = this.createDepthResources();
      ret.framebuffers = this.createFramebuffers(ret.imageViews, ret.depthResources.imageView, ret
            .renderPass);
      ...
      ret.descriptorSets = this.createDescriptorSets(numImages, ret.descriptorPool, this.descriptorSetLayout, ret
            .uniformBuffers);
   ...
   void cleanupSwapchain(Swapchain sc)
   {
      ...
      vkFreeCommandBuffers(this.logicalDevice, this.commandPool, this.swapchain.GetNumImages(), sc
            .commandBuffers.ptr);
   ...
   in (m_state == RendererState.BetweenFrames)
   {
      ...
      check!vkAcquireNextImageKHR(
         ...
         this.swapchain.imageAvailableSemaphore,// Signalled when the image is grabbed. Note that if this function
   ...
   out (b; b.memory != VK_NULL_HANDLE)
   {
      ...
      VkBufferCreateInfo bufInfo = {
         size: ret.size,
         usage: usage,
         sharingMode: VK_SHARING_MODE_EXCLUSIVE,// ^^ I guess this is where you'd do buffer aliasing etc
   ...
   out (; m_state == RendererState.AcceptingCommands)
   {
      ...
      check!vkWaitForFences(this.logicalDevice, 1, &this.swapchain.commandBufferReadyFence, false, ulong
            .max);
   ...
   out (ret; ret.length == count) // out (ret; !ret.containsDuplicates)
   {
...
struct SwapchainSupportDetails
{
   ...
   static SwapchainSupportDetails Get(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface)
   {
      ...
      vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, surface, &numPresentModes, ret
            .presentModes.ptr);
...
)
{
   VkAttachmentDescription colourAttachment = {
      ...
      format: colourFormat,
      samples: VK_SAMPLE_COUNT_1_BIT,
      loadOp: VK_ATTACHMENT_LOAD_OP_CLEAR,// Clear image on load, so initialLayout doesn't matter
